...
public final class CarbonLocaleGenerator{
    ...
    private static void generateEnumStructure(List<String> lines){
        ArrayList<String> builder = new ArrayList();
        List<String> enums_list = parseList(lines);
        ...
        builder.add("}");
        generateCarbonLocales(builder);
    }
    
    /** Parses a csv file with a header like "locale,name,intensity". */
    public static List<String> parseList(List<String> lines) {
    ...
    private static void generateCarbonLocales(List<String> class_structure){

        try(BufferedWriter bw = Files.newBufferedWriter(Paths.get("src/jcarbon/src/main/java/jcarbon/emissions/CarbonLocale.java"), Charset.defaultCharset())){
            PrintWriter pw = new PrintWriter(bw);
            for(String e: class_structure){
    ...
    public static void main(String[] args){
        generateEnumStructure(List.of("ISO,Country,CarbonIntensity","ABW,Aruba,542.8195532899147", "AFG,Afghanistan,108.65829596412556", "AGO,Angola,175.60894163125997"));
